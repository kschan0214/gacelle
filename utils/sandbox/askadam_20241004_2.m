...
%
    ...
    methods
        ...
        % loss_reg      : loss associated with (TV) regularisation
            ...
            signal_FWD = FWDfunc(this.unscale_parameters(parameters,fitting.lb,fitting.ub,fitting.model_params),varargin{:});
            ...
            if fitting.lambda{1} > 0
                for kreg = 1:numel(fitting.lambda)
                    ...
                    cost        = this.reg_TV(utils.reshape_ND2image(parameters.(fitting.regmap{kreg}).',mask),mask,fitting.TVmode,fitting.voxelSize);
        ...
        %
            ...
            if ~ismatrix(data);     data    = utils.reshape_NDto2Dinput(data,      mask_idx); else; data = data(:,mask_idx);     end
            if ~ismatrix(weights);  weights = utils.reshape_NDto2Dinput(weights,   mask_idx); elseif ~isempty(weights); weights = weights(:,mask_idx);  end
            ...
            parameters         = this.set_boundary01(parameters);           parameters          = utils.reshape_2DinputtoND_struct(  parameters,mask);
            parameters_minLoss = this.set_boundary01(parameters_minLoss);   parameters_minLoss  = utils.reshape_2DinputtoND_struct(parameters_minLoss,mask);
        ...
        function parameters = initialise_parameter(this,img_size,pars0,fitting,mask)
            ...
            % masking
            parameters = structfun(@transpose,utils.gpu_vectorise_NDto2D_struct(parameters,mask),'UniformOutput',false);
