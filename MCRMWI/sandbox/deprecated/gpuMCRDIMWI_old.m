...
% Date modified:
    ...
    methods
        %% Constructor
        function obj = gpuMCRDIMWI(te,tr,fa,fixed_params)
            obj.te = double(te(:));
            obj.tr = double(tr(:));
            obj.fa = double(fa(:));
        ...
        % Forward model to generate MCRMWI signal
            ...
            if ~DIMWI.isFitFreqMW || ~DIMWI.isFitFreqIW
                ...
                % derive g-ratio 
                g = hcfm_obj.gratio(abs(s0iw),abs(pars.s0mw)/obj.rho_mw);
            ...
            if ~DIMWI.isFitR2sEW
                ...
                fvf = hcfm_obj.FibreVolumeFraction(s0iw,s0ew,abs(pars.s0mw)/obj.rho_mw);
            ...
            % main
            totalVolume = s0iw + s0ew + pars.s0mw / obj.rho_mw;
            mvf         = (pars.s0mw / obj.rho_mw) ./ totalVolume;
