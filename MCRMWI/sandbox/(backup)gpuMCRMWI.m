...
% Date modified:
    ...
    methods
        ...
        function [algoPara,data_obj_all] = data_preparation(obj,algoPara,imgPara)
            ...
            % exclude CSF as ANN did not train with CSF values
            % mask_noncsf = t1iew0 < 2.5;
            mask_noncsf = 1;

            mask_nonzero = min(data(:,:,:,1,:) ~= 0,[],5);

            % Max intensity should be on 1st echo 
            % [~,idx] = max(abs(data),[],4);
            % mask_max1stecho = min(idx == 1,[],5);
            mask_max1stecho = ones(size(t1iew0));
            
            % final mask for fitting
            mask = and(and(and(and(and(and(mask,mask_valida_r2s),mask_valid_m0),mask_valid_r1iew0),mask_nonzero),mask_noncsf),mask_max1stecho);
                                    ...
                                    for kfield2 = 1:numel(fieldname2)
                                        if numel(temp.res_obj.(fieldname{kfield}).(fieldname2{kfield2})) ~= 1
                                            dims = [size(temp.res_obj.(fieldname{kfield}).(fieldname2{kfield2}),1),size(temp.res_obj.(fieldname{kfield}).(fieldname2{kfield2}),2),...
                                                Nbatch,size(temp.res_obj.(fieldname{kfield}).(fieldname2{kfield2}),3),size(temp.res_obj.(fieldname{kfield}).(fieldname2{kfield2}),4),...
                                                size(temp.res_obj.(fieldname{kfield}).(fieldname2{kfield2}),5)];
        ...
        %
            ...
            if algoPara.isComplex
                fitRes.final.Freq_BKG   = permute(single(gather(extractdata((parameters.totalField_plus + extra_data.totalField).* mask(:,:,:,1,1)))),[1 2 3 5 4]);
            ...
            if algoPara.isComplex
                fitRes.min.Freq_BKG   = permute(single(gather(extractdata((parameters_minLoss.totalField_plus + extra_data.totalField).* mask(:,:,:,1,1)))),[1 2 3 5 4]);
        ...
        function [gradients,loss,loss_fidelity,loss_reg] = modelGradients(obj, parameters, y, true_famp, mask, weights, extra_data,Nsample, ub, lb,fitParams,dlnet)
           ...
           % rescale network parameter to true values
            ...
            if fitParams.isComplex
                totalfield  = extra_data.totalField + parameters_true.totalField_plus;
        ...
        % Forward model to generate MCRMWI signal
            ...
            if DIMWI.isFitVic
                ...
                % % if ICVF is provided from DWI
                % s0ew = pars.s0iew .* (1-extra_data.icvf);
                % s0iw = pars.s0iew .* (extra_data.icvf);
            ...
            if ~DIMWI.isFitR2sEW
                ...
                fvf     = hcfm_obj.FibreVolumeFraction(abs(s0iw),abs(s0ew),abs(s0mw)/obj.rho_mw);
        ...
        function [ss_pool] = model_BM_2T1(obj,f,R1f,kfr,true_famp)
        ...
        % ss_pool: 1st-3rd dim:image; 4th dim: TE;5th dim: FA;6th dim: [S_M,S_IEW]
            ...
            sf = ((f./2 - 1./2).*(kfr.*sin(2.*true_famp).*exp((TR.*(kfr + R1f.*f + R1r.*f - ...
            ...
            sr = -(2.*f.*exp((TR.*(kfr + R1f.*f + R1r.*f + (R1f.^2.*f.^2 - 2.*R1f.*R1r.*f.^2 + 4.*R1f.*f.^2.*kfr - 2.*R1f.*f.*kfr + ...
    ...
    methods(Static)
        ...
        function [parameters,extra_data] = initialise_model(data_obj,algoPara,ub,lb)
            ...
            totalField_plus = (zeros(size(data_obj.fm0),'single')         -lb(11)) / (ub(11)-lb(11));
            ...
            if algoPara.isComplex
                parameters.totalField_plus  = gpuArray( dlarray( permute(totalField_plus,[1 2 3 5 4])) );
                parameters.pini             = gpuArray( dlarray(pini) );
            else
                extra_data.pini         = gpuArray( dlarray(pini * (ub(12)-lb(12))+lb(12)) );
            end
            extra_data.totalField   = gpuArray( dlarray(permute(data_obj.fm0,[1 2 3 5 4]) ) );
        ...
        function parameters = rescale_parameters(parameters,fitParams,ub,lb)
            ...
            if fitParams.isComplex
                parameters.totalField_plus   = parameters.totalField_plus  * (ub(11)-lb(11)) + lb(11);
