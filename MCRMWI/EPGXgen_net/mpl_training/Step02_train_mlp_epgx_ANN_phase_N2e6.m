%% Step02_train_mlp_epgx_ANN_phase_N2e6.m
%
...
output_prefix       = 'MCRMWI_MLP_EPGX_RFphase50_T1M234_phase';
...
% Order of output parameter: angle(ss_iew)
...
% create response variable, phase of IEW
response = angle(output_parameter(:,:,1));            % phase of FW w/ EPGX

% create datastore
inds   = arrayDatastore(features.', 'IterationDimension',2);
outds  = arrayDatastore(response.', 'IterationDimension',2);

clear input_parameter output_parameter features response response
...
%% validation part 1
load(fullfile(output_dir,'MCRMWI_MLP_EPGX_RFphase50_T1M234_phase.mat'));

load(fullfile(epgx_dictionary_dir,['EPGX_steadystate_batch-' num2str(45) '.mat']));

nfeatures = 11;
features_valid = zeros(size(input_parameter,1),40,nfeatures, 'single');
for kfa = 1:40
features_valid(:,kfa,:) = feature_preprocess_MCRMWI_MLP_EPGX_leakyrelu(input_parameter(:,1),input_parameter(:,2),...
                                              input_parameter(:,3),d2r(fa(kfa)),input_parameter(:,4),input_parameter(:,5),T1Myelin).';
end
response_valid = single(angle(output_parameter(:,:,1)));     
