...
% Date modified:
    ...
    methods
        ...
        % constructor
        function this = gpuTFI(voxelSize,B0,B0dir,te)
        ...
        %
            ...
            pars0                   = this.determine_x0(f_Hz,fitting,extraData);

            %%%%%%%%%%%%%%%%%%%% End 1 %%%%%%%%%%%%%%%%%%%%

            %%%%%%%%%%%%%%%%%%%% 2. Setting up all necessary data, run askadam and get all output %%%%%%%%%%%%%%%%%%%%
            % 2.1 setup fitting weights
            w = this.compute_optimisation_weights(extraData,f_Hz,fitting);
            ...
            out = askadam().optimisation(f, mask, w, pars0, fitting, userFcn, userInput);
            % out = askadam().optimisation(f, mask, w, pars0, fitting, @this.FWD, mask, fitting, extraData);

            out.final = this.postprocess(out.final, f_Hz, extraData, fitting);
            out.min   = this.postprocess(out.min,   f_Hz, extraData, fitting);
        ...
        % Anyone recomputing chi from y will find a constant offset.
            ...
            s.residualField = f_Hz - fittedField_Hz;
        ...
        % converging to the wrong wrap.
            ...
            if isfield(extraData,'chi0') && ~isempty(extraData.chi0)
                fprintf('Using supplied chi0 / P as starting point.\n');
                x0.y = extraData.chi0 ./ max(extraData.P, eps('single'));
            else
                fprintf('Using default starting points for all voxels at [%s]: [%s]\n', ...
                    cell2str(this.modelParams),replace(num2str(this.startPoint(:).',' %.2f'),' ',','));
                if fitting.isnonlinear
                    warning(['Zero starting point with the nonlinear fidelity. The total field ' ...
                             'spans many wraps; supply extraData.chi0 unless you have a reason not to.']);
                end
        ...
        function [f] = FWD(this, pars, fitting, extraData)
        % Forward model: preconditioned susceptibility -> field.
        %
        ...
        %
        % Unlike gpuPDF there is NO support constraint here and no mask
        % argument: chi is estimated over the whole FOV, including air, bone
        % and scalp. Restriction to the measured region happens only through
        % the fidelity mask and weights inside askadam.m.

            chi = extraData.P .* pars.y;                                % [ppm], whole FOV

            % dipole convolution in k-space; D is dimensionless so f stays in ppm
            f   = real(gacelleFFT.ifft3s_(extraData.D .* gacelleFFT.fft3s_(chi)));
        ...
        function [ data, mask, extraData ] = prepare_data(this, data, mask, extraData, fitting)
            ...
            if ~isfield(extraData,'MG') || isempty(extraData.MG)
                img             = sqrt(sum(abs(extraData.img).^2,4));
