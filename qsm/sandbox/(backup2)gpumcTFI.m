...
% Date modified:
    ...
    % y     : preconditioned susceptibility [ppm / P]
    % Ar,Ai : complex amplitude at t = 0, in units of the normalised data
    % R2s   : effective transverse relaxation rate [Hz]
    %
    % modelParams{k} <-> ub(k) <-> lb(k) <-> startPoint(k)
    % These MUST stay the same length and index-aligned.
    %
    % Bounds under the sigmoid transform: dtheta/dz = (theta-lb)(ub-theta)/(ub-lb),
    % which is ZERO at either bound. startPoint must therefore never sit on
    % a bound, and bounds should span roughly 4x the expected operating
    % range so the working point stays mid-sigmoid.
    %
    % Ar/Ai at +/-4 rather than +/-1.5: the data is normalised so |A| ~ 1,
    % and widening the bounds INCREASES dtheta/dz (the (ub-lb)/4 factor
    % grows faster than sigma(1-sigma) shrinks). At +/-4 the gradient is
    % roughly flat across the operating range; at +/-1.5 it is already
    % falling at |A| = 1.
    %
    % R2s is NOT rescaled. Under the sigmoid transform the native step is
    % lr*dtheta/dz, which is scale invariant - fitting R2s/100 with bounds
    % [0,2] gives exactly the same native step as R2s with [0,200]. If R2s
    % stalls, the cause is a zero gradient (wiring) or Adam's epsilon floor,
    % not the parameter scale.
        modelParams     = { 'S0';  'R2s'; };
        ub              = [    4;    200;   ];
        lb              = [    0;    0.5;  ];
        startPoint      = [    1;     20; ];
    ...
    methods
        ...
        %
            ...
            pars0                   = this.determine_x0(data,fitting,extraData);
        ...
        function s = postprocess(this, s, f_Hz, extraData, fitting)
            ...
            % --- crop last ---
            s.y             = crop_padding(s.y,            extraData);
        ...
        function x0 = determine_x0(this,y,fitting,extraData)
        ...
        function pars0 = estimate_prior(this,data,mask)
            ...
            start   = tic;
            % Sn      = data(:,:,:,1) ./ max(abs(data(:,:,:,1)), eps('single'));
            % pars0.c = real(Sn);   pars0.s = imag(Sn);
        ...
        % vein (should be ~ +0.3 ppm) rather than reasoning about it.
            ...
            % 
            % 
            % s = abs(pars.S0.*exp(-pars.R2s .* t).*exp(1i*2*pi*this.gyro*this.B0.*f_ppm .* t + pars.phi));

            % s = cat(5,pars.S0.*exp(-pars.R2s .* t).*cos(2*pi*this.gyro*this.B0.*f_ppm .* t + pars.phi),...
            %           pars.S0.*exp(-pars.R2s .* t).*sin(2*pi*this.gyro*this.B0.*f_ppm .* t + pars.phi));

            % % accumulated phase at each echo
            % th    = (2*pi*this.gyro*this.B0) .* t .* f_ppm;     % rad
            % ct    = cos(th);
            % st    = sin(th);
            % 
            % % magnitude decay - the term the unit-phasor model discards
            % decay = exp(-pars.R2s .* t);
            % 
            % s = cat(5, decay .* (pars.Ar.*ct - pars.Ai.*st), ...
            %            decay .* (pars.Ar.*st + pars.Ai.*ct));
        ...
        function [ data, mask, extraData ] = prepare_data(this, data, mask, extraData, fitting)
            ...
            scale = prctile(s0(and(mask,isfinite(s0))),99);
            data  = data./scale;
            ...
            [padPre, padPost] = check_padsize(mask, 20, this.voxelSize);
            ...
            if any([padPre padPost] > 0)
                ...
                if isfield(extraData,'MG');      extraData.MG       = pf(extraData.MG, 0); end
                if isfield(extraData,'M2');      extraData.M2       = pf(extraData.M2, false); end
                if isfield(extraData,'P');       extraData.P        = pf(extraData.P, this.Ps); end
            ...
            switch lower(fitting.precond)
                ...
                case 'emp+r2s'
                    ...
                    if ~isfield(extraData,'R2s')
                        R2s = this.R2star_trapezoidal(extraData.img,this.te);
                    else
                        R2s = extraData.R2s;
                    end
            ...
            if ~isequal(size(extraData.P), matrixSize)
                error('gpuTFI:sizeMismatch','extraData.P must be the same size as the field map.');
            end
