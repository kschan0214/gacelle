...
% Date modified:
    ...
    methods
        ...
        % constructor
        function this = gpuMETFIv2(voxelSize,B0,B0dir,te)
        ...
        function s = postprocess(this, s, f_Hz, extraData, fitting)
            ...
            s.phi           = crop_padding(s.phi,          extraData);
        ...
        function s = FWD(this, pars, fitting, extraData)
            ...
            chi   = extraData.P .* pars.y;                                       % ppm
            f_ppm = real(gacelleFFT.ifft3s_(extraData.D .* gacelleFFT.fft3s_(chi)));
        
            % per-echo phase; te is [1,1,1,nTE] in seconds
            th = (2*pi*this.gyro*this.B0) .* permute(this.te(:),[2 3 4 1]) .* f_ppm;              % check sign
        
            ct = cos(th);  st = sin(th);
            c = cos(pars.phi); s = sin(pars.phi);
        
            s = cat(5, c.*ct - s.*st, ...
                       c.*st + s.*ct);                                 % [x,y,z,nTE,2]
        end
        ...
        function cost = reg_phi(this, pars, mask, extraData)

            % compute gradients
            grad_spatial    = this.fgrad(pars.phi, this.voxelSize);

            % isotropic TV
            % P^2-scaled numerical conditioning (see epsTV0 above)
            gmag            = sqrt(sum((mask .* grad_spatial).^2,4) + this.epsTV0);
        ...
        function cost = regulariser(this, pars, mask, fitting, extraData)
            ...
            R_phi = this.reg_phi( pars, mask, extraData);

            % concatenate all regularisation terms
            cost = fitting.lambdaTV*R_TV + fitting.lambdaCSF*R_CSF + fitting.lambdaPhi * R_phi;
