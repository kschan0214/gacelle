...
% Date modified: 
    properties
        ...
        modelParams     = {'Rs';'fs';'fa';'Da';'De';'ra';'p2'};
        ub              = [  20;   1;   1;   3;   3;   1;  1];
        lb              = [ eps; eps; eps; eps; eps;1/250; eps];
        startPoint      = [   7; 0.2; 0.4;   2;   1; 0.05; 0.2];
    end
    ...
    methods
        ...
        % constructuor
        function this = gpuSANDIX(b, ldelta, BDelta, Ds, varargin)
        ...
        function display_data_model_info(this)
            ...
            disp('==========================');
            disp('SANDIX with askAdam solver');
            disp('==========================');
        ...
        % Estimation starting points for NEXI using likehood method
            ...
            pars0_csf = [0.1,0,0.01,1,1,0.01,0.01];
            for k = 1:size(pars,4)
                tmp                 = pars(:,:,:,k);
        ...
        function [x_train, S_train, intervals] = traindata(this, N_samples, lmax, varargin)
            ...
            for k = 1:reps
                ...
                for j = 1:batch_size
                    Sl0(:,j) = this.Sl0(pars(1,j), pars(2,j), pars(3,j), pars(4,j), pars(5,j), pars(6,j));
                end
        ...
        function S = Sl0(this, Rs, fs, fa, Da, De, ra)
            ...
            % 1. Intra-soma signal, Murday and Cotts, JCP, 1968
            C  = this.MurdayCotts(Rs, this.Ds, g, delta, BDELTA);    % less memory efficient but faster
            Ss = exp(-C);
            
            % 2. exchanging intracellular and extracellular signal
            Da = bval.*Da;
            De = bval.*De;
            ra = BDELTA.*ra;
            re = ra.*fa./(1-fa);
            ...
            Sie = trapz(x(:),this.M(x, fa, Da, De, ra, re),ndims(x));
    ...
    methods(Static)
        ...
        function s = MurdayCotts(r, Ds, g, delta, Delta)
