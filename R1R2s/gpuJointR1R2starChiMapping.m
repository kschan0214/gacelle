classdef gpuJointR1R2starChiMapping < handle
...
% Date modified: 
    ...
    properties
        ...
        % R1    : (=1/T1) in s^-1
        % R2star: R2* in s^-1   
        % Chi   : magnetic susceptibility in ppm
        modelParams     = {'M0';'R1';'R2star';'Chi';'dpini'};
        ub              = [   2;  10;     200;  0.4;   pi/2];
        lb              = [   0; 0.1;     0.1; -0.4;  -pi/2];
        startPoint      = [   1;   1;     30;  1e-8;  1e-8];
    end
    ...
    methods
        ...
        % constructuor
        function this = gpuJointR1R2starChiMapping(te,tr,fa,B0,B0dir)
        ...
        function display_data_model_info(this)
            ...
            disp('============================================================');
            disp('Joint R1-R2*-QSM mapping with vFA-mGRE data - askAdam solver');
            disp('============================================================');
        ...
        %
            ...
            iMag                    = sqrt(sum(sum(abs(data).^2,4),5));
            grad                    = @cgrad;
            wG                      = gradient_mask(gradient_weighting_mode, iMag, mask, grad, fitting.voxelSize);
            wG                      = wG.*mask;

            r2s         = this.R2star_trapezoidal(mean(abs(data),5),this.te);    % 20250628: correct names
            mask_CSF    = extract_CSF(r2s,mask,fitting.voxelSize)>0;

            TVmode      = '3D';
            lamdaPhi    = fitting.lambda{1};
            lambdaChi   = fitting.lambda{2};
            lambdaCSF   = fitting.lambda{3};
            % regInput    = {mask_lf, lambda, regmap, TVmode, voxelSize,lambdaCSF,mask_CSF};
            regInput    = {lamdaPhi,gpuArray(single(mask)),lambdaChi, gpuArray(single(wG)),lambdaCSF,gpuArray(single( mask_CSF )), TVmode, fitting.voxelSize, []};
            ...
            out         = askadamObj.optimisation(data, mask, w, pars0, fitting, userFcn,userInput);
        ...
        function [s] = FWD(this, pars, fitting, extraData)
            ...
            R1      = pars.R1;
            R2star  = pars.R2star;
            freqBKG = extraData.freqBKG + real(this.ifft3s_(extraData.dipoleKernel .* this.fft3s_(Chi)));
            % R2star(R1>R2star) = R1(R1>R2star);
            
            s = this.model_jointR1R2s(M0, R2star, R1, TE,this.tr,trueFlipAngle);

            Sreal = s .* cos(TE .* 2.*pi.*freqBKG.*this.B0.*this.gyro + pini);
            Simag = s .* sin(TE .* 2.*pi.*freqBKG.*this.B0.*this.gyro + pini);
